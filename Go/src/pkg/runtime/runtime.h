...
struct	G
{
	byte*	stackguard;	// cannot move - also known to linker, libmach, runtime/cgo 不能删除，在其他文件用到
	byte*	stackbase;	// cannot move - also known to libmach, runtime/cgo 不能删除，在其他文件用到
	Defer*	defer;	// defer函数链表
	Panic*	panic;	// panic函数栈链表
	Gobuf	sched;	// g缓存池管理，也就是sched
...
struct	M
{
	...
	int32	profilehz;
	int32	helpgc;		// 告诉gc回收
	uint32	fastrand;	// TODO：不知道什么意思，在proc.c的mcommoninit中做初始化
	uint64	ncgocall;
	Note	havenextg;
	G*	nextg;
	M*	alllink;	// on allm
	...
	MCache	*mcache;			// 每个m有一个mcache，具体对应tcmalloc中的thread_cache。小对象的内存分配就在这里面做
	FixAlloc	*stackalloc;	// 已分配的空间
	G*	lockedg;
	G*	idleg;
	uintptr	createstack[32];	// Stack that created this thread.	创造线程的函数堆栈
