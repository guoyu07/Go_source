...
M	runtime·m0;	// m0
G	runtime·g0;	// idle goroutine for m0  这是闲置的g0
...
// Go调度器的工作就是将准备被运行的goroutine和准备工作的machine进行调度结合。如果有g却没有在等候的m
// ready()函数就会在一个新的系统线程中开启一个m，这样准备中的g才能同时被运行，g同时被运行的条数会越来
// 越多，直到上限。换句话说，程序中永远不会没有m
//
...
// Go默认同一时间只有一个内核线程（m）在运行；其他的线程会被操作系统阻塞住。如果你设置了环境变量
// $GOMAXPROCS或者调用了runtime.GOMAXPROCS()，这两个行为都会改变同一时间并行的用户线程数量。因此，
// $GOMAXPROCS应该是一个和使用的机器最大核数相近的数字。
//
...
// TODO:这段翻译不好
// 一个单进程且无死锁的程序可能会拥有多个m。比如初期的时候的m可能和运行最繁忙时候的m一样多（当然是
// 有上限 runtime.sched.mmax的），程序允许在不同的事件使用并行来运行。我们再看看这个选择，只有踢掉
// 阻塞住系统调用的m才行。但是这样的话就有可能限制了并行的数量。但go仍然是这么去做的。
//
// In general, one could imagine all sorts of refinements to the
// scheduler, but the goal now is just to get something working on
// Linux and OS X.
// 你当然也可以为scheduler设定更好的结构和定义，但是在这里，它的目标只是为了让程序在linux和os x上运行
// 而已

struct Sched {
	Lock;	// 锁

	G *gfree;	// available g's (status == Gdead) 状态是Gdead的g
	int32 goidgen; // goroutine的id生成器

	G *ghead;	// g's waiting to run 等待运行的g链表头部
	G *gtail;	// 等待运行的g链表尾部
	int32 gwait;	// number of g's waiting to run 等待运行的g链表中goroutine的数目
	int32 gcount;	// number of g's that are alive 非死亡状态的g数目
	int32 grunning;	// number of g's running on cpu or in syscall 在cpu或者系统中运行的g数目

	M *mhead;	// m's waiting for work 等待使用的m链表
	int32 mwait;	// number of m's waiting for work 等待使用的m数目
	int32 mcount;	// number of m's that have been created 被创建的m数目

	volatile uint32 atomic;	// atomic scheduling word (see below) 位设置（看下面的说明）

	int32 profilehz;	// cpu profiling rate cpu状态概要比率

	bool init;  // running initialization 是否正在初始化
	bool lockmain;  // init called runtime.LockOSThread 是否调用LockOSThread

	Note	stopped;	// one g can set waitstop and wait here for m's to stop
						// g可以设置等候暂停，并在这里等着m来暂停
};

// The atomic word in sched is an atomic uint32 that
// holds these fields.
// sched的位设置是一个32位的数字，它的每个位代表下面
//
...
//
//	[15位] mcpu  		在cpu中执行的m个数
//	[15位] mcpumax 		在cpu中最大允许执行的m的个数
//  [1位]  waitstop 	是否有g在stopped中等候
//	[1位] gwaiting 		等待运行的g个数是否大于0
//
...
// 这些属性是被系统用来决定是否和调度器交互用的。使用系统的一个机器字来存储它们，这是最简单的实现
// 不用锁实现读写操作的方法。这个方法在很好解决了系统调用的冲突 - 和多线程程序交互
//
...
// 除了entersyscall和exitsyscall之外有机会改变这些字段的就是lock操作了。所以得到锁的程序只需要考虑
// entersyscall和exitsyscall的调用是否会影响，而不需要考虑其他得到锁的程序。
//
...
// 具体来说，entersyscall和exitsyscall只读取mcpumax，waitstop和gwaiting。但是却从不写这些值。因此，
// 当拥有锁的程序对这些值进行写操作的时候就不需要考虑写冲突的情况了。但是，逻辑冲突仍然是无法避免的：
// 比如，waitstop必须在mcpu大于mcpumax才有可能触发，否则这里的wait的g就有可能是假睡眠。这Promela模型
// 在proc.p中验证了这些需求。
enum {
	mcpuWidth = 15,   // mcpu的宽度，前面有说是15
	mcpuMask = (1<<mcpuWidth) - 1, // mcpu的Mask，2^15-1
	mcpuShift = 0, // mcpu偏移量
	mcpumaxShift = mcpuShift + mcpuWidth, // mcpumax的偏移量
	waitstopShift = mcpumaxShift + mcpuWidth, // waitstop的偏移量
	gwaitingShift = waitstopShift+1, // gwaiting的偏移量
	...
	// beyond zero.
	// GOMAXPROCS的上限是我们在mcpuWith中能存的最大量（其实在它的基础上减少了10）。
...
// TODO:这段有问题
// 这个m是等候处理notewakeup中的g。只有当调度器的lock被取到才能做操作。当取到调度器
// 锁的时候，这个m能用来使调用notewakeup的g的时间最小化。因为m会自动地快速移动，然后
// 自我锁住，造成锁争夺。
static M* mwakeup;

// Scheduling helpers.  Sched must be locked.
// 调度器helper，sched结构必须被锁住
static void gput(G*);	// put/get on ghead/gtail 把g放在ghead或者gtail中
static G* gget(void);	// 获取一个等待运行的g
static void mput(M*);	// put/get on mhead 把m放在mhead上
static M* mget(G*);		// 获取一个可以使用的m
static void gfput(G*);	// put/get on gfree 把g放在gfree上
static G* gfget(void);	// 获取gfreee的一个g
static void matchmg(void);	// match m's to g's 将g和m匹配起来
static void readylocked(G*);	// ready, but sched is locked 准备好，并锁住sched
static void mnextg(M*, G*);		// 指定m要处理的下个g
static void mcommoninit(M*);	// m公共特性的初始化
...
setmcpumax(uint32 n)
{
	...
	for(;;) {
		...
		w = v;
		w &= ~(mcpuMask<<mcpumaxShift);	// 将mcpumax位置清0
		w |= n<<mcpumaxShift;	// 将mcpumax位置设置为n
		if(runtime·cas(&runtime·sched.atomic, v, w))	// 将atomic值替换为转换后的w
			break;
	}
}
...
// 启动的流程是：
// 调用osinit (thread_XXX.c)
// 调用schedinit 
// 建立新的G
// 调用runtime·mstart (proc.c)
void
runtime·schedinit(void)
{
	...
	runtime·goargs();	// 获取参数放在os·Args
	runtime·goenvs();	// 获取环境变量放在syscall·envs
	...
	if(p != nil && (n = runtime·atoi(p)) != 0) {
		if(n > maxgomaxprocs)
			n = maxgomaxprocs;	// 不能大于gomaxprocs的上限
		runtime·gomaxprocs = n;
	}
	// wait for the main goroutine to start before taking
	// GOMAXPROCS into account.
	// 主goroutine调用的时候先设置为1
	setmcpumax(1);
	runtime·singleproc = runtime·gomaxprocs == 1;

	canaddmcpu();	// mcpu++ to account for bootstrap m  mcpu自增来启动m
	m->helpgc = 1;	// flag to tell schedule() to mcpu--  告诉gc回收
	runtime·sched.grunning++;	// sched正在运行的g增加1（g0）

	mstats.enablegc = 1;	// mstats标记为可以使用gc
...
// 是否能增加m的cpu数目，当可以增加mcpu的数目的时候，就执行增加操作
static bool
canaddmcpu(void)
{
	uint32 v;
...
runtime·mstart(void)
{
	...
	// 存储被mcall使用的stack栈顶元素，一旦启动了调度，就不会回头调用了。
	// 所以，其他的调用可以重用这个栈空间
	runtime·gosave(&m->g0->sched);
	m->g0->sched.pc = (void*)-1;  // make sure it is never used 确定从未被使用
	runtime·asminit();	// asm的初始化
	runtime·minit();

	// Install signal handlers; after minit so that minit can
	// prepare the thread to be able to handle the signals.
	// 安装信号处理器，在minit之后，来准备处理m信号了
	if(m == &runtime·m0)
		runtime·initsig();
...
// Never returns.
// scheduler的作用：找到goroutine并且运行它。
// 有个争论的地方就是当goroutine在schedule之前被调用的时候，或者schedule第一次被调用
// 这个函数就用户不会返回了
static void
schedule(G *gp)
{
